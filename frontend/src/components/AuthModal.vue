<script setup>
import { ref, reactive, watch } from 'vue';
import { useAuth } from '../store/auth.js';
import { 
  loginWithEmail, 
  registerWithEmail, 
  loginWithGoogle as googleLogin, 
  logout, 
  sendVerificationEmail 
} from '../firebase.js';
import { updateProfile } from 'firebase/auth';
import { Mail, Lock, User, X, Loader2, AlertCircle, CheckCircle2, Send } from 'lucide-vue-next';

const props = defineProps({
  isOpen: {
    type: Boolean,
    required: true
  }
});

const emit = defineEmits(['close']);
const { authState } = useAuth();

// Abas: 'login', 'register' ou 'verify' (tela pós-cadastro)
const activeTab = ref('login');
const isSubmitting = ref(false);
const errorMessage = ref('');
const successMessage = ref('');
const showResendBtn = ref(false);

const formData = reactive({
  name: '',
  email: '',
  password: ''
});

// Limpa formulários e mensagens ao abrir/fechar
watch(() => props.isOpen, (newVal) => {
  if (!newVal) {
    formData.name = '';
    formData.email = '';
    formData.password = '';
    activeTab.value = 'login';
    errorMessage.value = '';
    successMessage.value = '';
    showResendBtn.value = false;
  }
});

const handleAuth = async () => {
  isSubmitting.value = true;
  errorMessage.value = '';
  successMessage.value = '';
  showResendBtn.value = false;

  try {
    if (activeTab.value === 'login') {
      // 1. Tenta realizar o Login
      const userCredential = await loginWithEmail(formData.email, formData.password);
      const user = userCredential.user;

      // 🛑 BLOQUEIO: Se o e-mail não foi verificado
      if (!user.emailVerified) {
        showResendBtn.value = true;
        await logout(); // Desconecta por segurança
        throw { code: 'auth/email-not-verified' };
      }

      // ✅ Sucesso no Login
      emit('close');

    } else {
      // 2. Cadastro de Novo Usuário
      if (formData.password.length < 6) {
        throw new Error('A senha deve conter no mínimo 6 caracteres.');
      }

      // O registerWithEmail já dispara o e-mail de verificação automaticamente
      const userCredential = await registerWithEmail(formData.email, formData.password);

      if (formData.name && userCredential.user) {
        await updateProfile(userCredential.user, {
          displayName: formData.name
        });
      }

      // Desconecta e direciona para a tela de confirmação
      await logout();
      activeTab.value = 'verify';
    }

  } catch (err) {
    console.error('Erro de autenticação:', err);

    switch (err.code) {
      case 'auth/email-not-verified':
        errorMessage.value = 'Seu e-mail ainda não foi verificado. Por favor, confira sua caixa de entrada.';
        break;
      case 'auth/email-already-in-use':
        errorMessage.value = 'Este e-mail já está cadastrado.';
        break;
      case 'auth/invalid-credential':
      case 'auth/wrong-password':
      case 'auth/user-not-found':
        errorMessage.value = 'E-mail ou senha incorretos.';
        break;
      case 'auth/weak-password':
        errorMessage.value = 'A senha deve ter no mínimo 6 caracteres.';
        break;
      default:
        errorMessage.value = err.message || 'Ocorreu um erro ao processar. Tente novamente.';
    }
  } finally {
    isSubmitting.value = false;
  }
};

// Reenviar E-mail de Verificação (Caso o usuário peça)
const handleResendVerification = async () => {
  isSubmitting.value = true;
  errorMessage.value = '';
  successMessage.value = '';

  try {
    // Faz o login temporário silencioso para pegar o usuário e reenviar o e-mail
    const userCredential = await loginWithEmail(formData.email, formData.password);
    await sendVerificationEmail();
    await logout();
    
    successMessage.value = 'E-mail de verificação reenviado com sucesso! Confira sua caixa de entrada e Spam.';
  } catch (err) {
    console.error('Erro ao reenviar e-mail:', err);
    errorMessage.value = 'Não foi possível reenviar o e-mail. Verifique suas credenciais.';
  } finally {
    isSubmitting.value = false;
  }
};

const handleGoogleLogin = async () => {
  isSubmitting.value = true;
  errorMessage.value = '';
  try {
    await googleLogin();
    emit('close');
  } catch (err) {
    console.error('Erro no Google Login:', err);
    errorMessage.value = 'Falha ao autenticar com o Google.';
  } finally {
    isSubmitting.value = false;
  }
};
</script>

<template>
  <div 
    v-if="isOpen" 
    class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4"
  >
    <!-- Backdrop com desfoque -->
    <div 
      @click="emit('close')"
      class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity"
    ></div>

    <!-- Modal Box -->
    <div class="relative bg-white w-full max-w-md rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-100 p-6 sm:p-8 z-10 transition-all transform scale-100">
      
      <!-- Botão Fechar -->
      <button 
        @click="emit('close')"
        class="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 rounded-xl hover:bg-slate-50 transition"
      >
        <X class="w-5 h-5" />
      </button>

      <!-- TELA DE SUCESSO DO CADASTRO (Aguardando Verificação) -->
      <div v-if="activeTab === 'verify'" class="text-center py-4">
        <div class="w-16 h-16 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-4 border border-emerald-100">
          <Mail class="w-8 h-8" />
        </div>
        <h3 class="text-xl font-bold text-slate-800 mb-2">Confirme seu E-mail</h3>
        <p class="text-sm text-slate-600 mb-6 leading-relaxed">
          Enviamos um link de confirmação para <strong class="text-slate-800">{{ formData.email }}</strong>.<br>
          Acesse sua caixa de entrada (ou pasta de Spam) para ativar sua conta antes de fazer o login.
        </p>
        <button 
          @click="activeTab = 'login'; errorMessage = ''"
          class="w-full bg-brand-600 hover:bg-brand-700 text-white font-semibold text-sm py-3 px-4 rounded-xl transition"
        >
          Ir para Tela de Login
        </button>
      </div>

      <!-- FORMULÁRIO DE LOGIN E CADASTRO -->
      <div v-else>
        <!-- Alternador de Abas -->
        <div class="flex border-b border-slate-100 mb-6">
          <button 
            @click="activeTab = 'login'; errorMessage = ''; successMessage = ''; showResendBtn = false"
            class="flex-1 pb-3 text-sm font-bold border-b-2 transition"
            :class="activeTab === 'login' ? 'border-brand-600 text-brand-600' : 'border-transparent text-slate-400 hover:text-slate-600'"
          >
            Entrar
          </button>
          <button 
            @click="activeTab = 'register'; errorMessage = ''; successMessage = ''; showResendBtn = false"
            class="flex-1 pb-3 text-sm font-bold border-b-2 transition"
            :class="activeTab === 'register' ? 'border-brand-600 text-brand-600' : 'border-transparent text-slate-400 hover:text-slate-600'"
          >
            Criar Conta
          </button>
        </div>

        <!-- Alertas de Erro -->
        <div 
          v-if="errorMessage" 
          class="mb-4 p-3.5 bg-red-50 border border-red-200 rounded-xl text-red-700 text-xs font-semibold flex flex-col gap-2"
        >
          <div class="flex items-center gap-2">
            <AlertCircle class="w-4 h-4 text-red-500 shrink-0" />
            <span>{{ errorMessage }}</span>
          </div>

          <!-- Botão para reenvio caso o e-mail não esteja verificado -->
          <button 
            v-if="showResendBtn" 
            @click="handleResendVerification" 
            type="button" 
            :disabled="isSubmitting"
            class="mt-1 text-xs font-bold text-brand-600 hover:underline flex items-center gap-1 self-start"
          >
            <Send class="w-3 h-3" />
            <span>Reenviar e-mail de confirmação</span>
          </button>
        </div>

        <!-- Alerta de Sucesso (Ex: Reenvio de E-mail) -->
        <div 
          v-if="successMessage" 
          class="mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-700 text-xs font-semibold flex items-center gap-2"
        >
          <CheckCircle2 class="w-4 h-4 text-emerald-500 shrink-0" />
          <span>{{ successMessage }}</span>
        </div>

        <form @submit.prevent="handleAuth" class="space-y-4">
          
          <!-- Campo Nome (Cadastro Apenas) -->
          <div v-if="activeTab === 'register'" class="relative">
            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 pl-1 block">Nome Completo</label>
            <div class="relative">
              <User class="absolute left-3 top-3 w-4 h-4 text-slate-400" />
              <input 
                v-model="formData.name"
                type="text" 
                required
                placeholder="Como prefere ser chamado?"
                class="w-full pl-9 pr-3 py-2.5 text-sm text-slate-800 placeholder-slate-400 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition"
              />
            </div>
          </div>

          <!-- Campo E-mail -->
          <div class="relative">
            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 pl-1 block">Endereço de E-mail</label>
            <div class="relative">
              <Mail class="absolute left-3 top-3 w-4 h-4 text-slate-400" />
              <input 
                v-model="formData.email"
                type="email" 
                required
                placeholder="exemplo@dominio.com"
                class="w-full pl-9 pr-3 py-2.5 text-sm text-slate-800 placeholder-slate-400 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition"
              />
            </div>
          </div>

          <!-- Campo Senha -->
          <div class="relative">
            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 pl-1 block">Senha de Acesso</label>
            <div class="relative">
              <Lock class="absolute left-3 top-3 w-4 h-4 text-slate-400" />
              <input 
                v-model="formData.password"
                type="password" 
                required
                placeholder="No mínimo 6 dígitos"
                class="w-full pl-9 pr-3 py-2.5 text-sm text-slate-800 placeholder-slate-400 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition"
              />
            </div>
          </div>

          <!-- Botão Entrar / Registrar -->
          <button 
            type="submit" 
            :disabled="isSubmitting"
            class="w-full bg-brand-600 hover:bg-brand-700 active:bg-brand-800 text-white font-semibold text-sm py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition duration-150 shadow-md shadow-brand-100 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Loader2 v-if="isSubmitting" class="w-4 h-4 animate-spin" />
            <span>{{ activeTab === 'login' ? 'Entrar na Conta' : 'Finalizar Cadastro' }}</span>
          </button>
        </form>

        <!-- Divisor -->
        <div class="my-6 flex items-center justify-center gap-3">
          <span class="h-px bg-slate-100 flex-1"></span>
          <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Ou continue com</span>
          <span class="h-px bg-slate-100 flex-1"></span>
        </div>

        <!-- Login Social Google -->
        <button 
          type="button"
          @click="handleGoogleLogin"
          :disabled="isSubmitting"
          class="w-full border border-slate-200 hover:bg-slate-50 text-slate-700 font-semibold text-sm py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 transition active:scale-[0.98] disabled:opacity-50"
        >
          <svg class="w-4 h-4" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" />
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" />
          </svg>
          <span>Acessar com o Google</span>
        </button>
      </div>

    </div>
  </div>
</template>