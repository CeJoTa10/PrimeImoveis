...
function getEmailTransporter() {
  if (process.env.SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS) {
    try {
      ...
      console.warn('[Nodemailer] Falha ao criar transportador SMTP:', err.message);
      return null;
    }
  }
  return null;
  ...
  try {
    ...
    if (db) {
      try {
        ...
        console.warn('[Firestore OTP Warning] Fallback local:', err.message);
...
export async function verifyCode(req, res) {
  try {
    ...
    if (!customToken) {
      console.log('[Auth Controller] Modo offline dev. Emitindo customToken.');
